 ...
 * @property {Array<import('./objects/ocrObjects.js').OcrLine>} lines - The lines of the cited rows.
 ...
 * @property {Array<number>} missing - Cited line numbers with no line on this printed page.
 */
 ...
 * @param {string} text
 * @returns {?{page: number, line: ?number, endPage: number, endLine: ?number}}
 */
...
export function resolveCitation(pages, text) {
  ...
  for (const p of printed) {
    if (Number.isNaN(p.num) || p.num < c.page || p.num > c.endPage) continue;
    ...
    if (c.line == null) {
      ...
      spans.push({
        page: p.page, pageNum: p.pageNum, first: nums.length ? Math.min(...nums) : null, last: nums.length ? Math.max(...nums) : null, lines, bbox, missing: [],
      });
      continue;
    }
    const lo = p.num === c.page ? c.line : 1;
    const hi = p.num === c.endPage ? (c.endLine != null ? c.endLine : c.line) : Infinity;
    ...
    spans.push({
      page: p.page, pageNum: p.pageNum, first: Math.min(...nums), last: Math.max(...nums), lines, bbox, missing,
    });
  }
  return spans;
}
