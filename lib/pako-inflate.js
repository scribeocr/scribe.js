 ...
 * @returns {Uint8Array}
 */
export function zlibInflate(data, meta) {
  ...
  const result = inflateRaw(data.subarray(offset), undefined, meta);
 ...
 */
export function inflateRaw(data, options, meta) {
  ...
  if (inflator.err) {
    if (inflator.result instanceof Uint8Array && inflator.result.length > 0) {
      if (meta) meta.recovered = true;
 ...
 * @param {{recovered?: boolean}} [meta] - set `recovered=true` if output was salvaged from a corrupted stream
 * @returns {Uint8Array}
 */
export function inflatePartial(data, meta) {
  try {
    return zlibInflate(data, meta);
  } catch { /* fall through */ }
  ...
  const out = salvagePartial(inflator) || new Uint8Array(0);
  if (meta && out.length > 0) meta.recovered = true;
  return out;
}
