...
export function ensureLayerStyleSheet() {
  ...
  styleEl.textContent = '.scribe-hide-text-layer .scribe-layer-text{display:none}'
    ...
    // The selection outline and handles hang off a frame child rather than the item, so the frame can stop shrinking below 26 screen px while the item bbox keeps its exact size.
    + '.scribe-item-frame,.scribe-item-pad{position:absolute;'
    + 'top:min(0px,calc((var(--scribe-item-h) - 26px/var(--scribe-zoom,1))/2));'
    + 'bottom:min(0px,calc((var(--scribe-item-h) - 26px/var(--scribe-zoom,1))/2));'
    + 'left:min(0px,calc((var(--scribe-item-w) - 26px/var(--scribe-zoom,1))/2));'
    + 'right:min(0px,calc((var(--scribe-item-w) - 26px/var(--scribe-zoom,1))/2))}'
    // The pad is the item's minimum tap and drag target, whether or not it is selected.
    + '.scribe-item-pad{pointer-events:auto}'
    + '.scribe-item-frame{pointer-events:none;outline:calc(1.5px/var(--scribe-zoom,1)) solid #1c62d4;'
    + 'outline-offset:calc(2px/var(--scribe-zoom,1))}'
    // The dots are 9px across including the white ring, centered on the outline corners.
    // The 2px outline-offset plus half the 1.5px line puts that corner 2.75px outside the item box.
    + '.scribe-item-dot{position:absolute;width:calc(6.5px/var(--scribe-zoom,1));height:calc(6.5px/var(--scribe-zoom,1));'
    + 'border-radius:50%;background:#1c62d4;border:calc(1.25px/var(--scribe-zoom,1)) solid #fff;pointer-events:auto}'
    + '.scribe-item-dot-nw{left:calc(-7.25px/var(--scribe-zoom,1));top:calc(-7.25px/var(--scribe-zoom,1));cursor:nwse-resize}'
    + '.scribe-item-dot-ne{right:calc(-7.25px/var(--scribe-zoom,1));top:calc(-7.25px/var(--scribe-zoom,1));cursor:nesw-resize}'
    + '.scribe-item-dot-sw{left:calc(-7.25px/var(--scribe-zoom,1));bottom:calc(-7.25px/var(--scribe-zoom,1));cursor:nesw-resize}'
    + '.scribe-item-dot-se{right:calc(-7.25px/var(--scribe-zoom,1));bottom:calc(-7.25px/var(--scribe-zoom,1));cursor:nwse-resize}'
    + '.scribe-coarse .scribe-item-dot::after{content:"";position:absolute;inset:calc(-9px/var(--scribe-zoom,1))}'
    + '.scribe-item-pulse{position:absolute;width:calc(44px/var(--scribe-zoom,1));height:calc(44px/var(--scribe-zoom,1));'
    + 'margin-left:calc(-22px/var(--scribe-zoom,1));margin-top:calc(-22px/var(--scribe-zoom,1));border-radius:50%;'
    + 'border:calc(2px/var(--scribe-zoom,1)) solid #1c62d4;pointer-events:none;opacity:0;'
    + 'animation:scribe-item-pulse .5s ease-out forwards}'
    + '@keyframes scribe-item-pulse{0%{transform:scale(.35);opacity:.9}100%{transform:scale(1);opacity:0}}'
    + '@media (prefers-reduced-motion: reduce){.scribe-item-pulse{animation:scribe-item-pulse-static .6s steps(1) forwards}}'
    + '@keyframes scribe-item-pulse-static{0%{transform:scale(.8);opacity:.7}100%{transform:scale(.8);opacity:0}}'
