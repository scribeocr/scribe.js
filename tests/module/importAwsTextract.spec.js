...
describe('Check AWS Textract JSON import correctly handles angle brackets.', () => {
  beforeAll(async () => {
    doc = await scribe.openDocument([`${ASSETS_PATH}/dictionary_spelling.png`,
  ...
  test('Should correctly import < and > signs from AWS Textract', async () => {
    ...
    const allText = doc.ocr.active[0].lines.map((line) => line.words.map((x) => x.text).join(' ')).join(' ');
...
describe('Check scribe JSON import handles null OCR pages (blank pages).', () => {
  test('Should import scribe JSON that has null entries in the OCR array without crashing', async () => {
    ...
    await scribe.terminate();
    doc = await scribe.openDocument({ scribeFiles: [buffer] });

    // Page 0 was null in the input, so it should be an empty placeholder page with default dims.
    expect(doc.ocr.active[0].lines.length).toBe(0);
    expect(doc.ocr.active[0].dims.width).toBe(1080);
    expect(doc.ocr.active[0].dims.height).toBe(1920);
    // Page 1 should be the original first page with real OCR data.
    expect(doc.ocr.active[1].lines.length > 0).toBe(true);
    expect(doc.ocr.active[1].lines[0].words[0].text).toBe('UNITED');
  });
...
describe('Check AWS Textract properly splits unicode superscript footnotes.', () => {
  beforeAll(async () => {
    doc = await scribe.openDocument([`${ASSETS_PATH}/econometrica_example_all_orientations.pdf`,
